...
defmodule SlurmRestAPI.Connection do
  @moduledoc """
  Handle Tesla connections for SlurmRestAPI.

  Additional middleware can be set in the compile-time or runtime configuration:

      config :tesla, SlurmRestAPI.Connection,
  ...
  def middleware(options \\ []) do
    base_url =
      Keyword.get(
        options,
        :base_url,
        Application.get_env(:slurmrestapi, :base_url, @default_base_url)
      )
    ...
    user_agent =
      Keyword.get(
        ...
        Keyword.get(
          tesla_options,
          :user_agent,
          "openapi-generator - SlurmRestAPI 0.0.39 - elixir"
        )
      )
